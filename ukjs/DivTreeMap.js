...
DivTreeMap.prototype.paint = function( displayNode, displayRect, level )
{
	...
	for( var i=0; i< nodeFacades.length; i++ )
	{
...
{	
	...
	var anim = new BoxAnimator( selected.div, this.getCoords(), this.animSteps, this.animDuration, function() { map.doZoom(); } );
	anim.animate();	
};
...
{
	//NB: it's tempting to use window.status here, but that doesn't get reflected in the GUI until this returns anyway ...
